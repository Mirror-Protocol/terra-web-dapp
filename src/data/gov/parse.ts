...
import { useGetProxyWhitelist } from "../contract/proxy"
...
export const useParsePoll = () => {
  const { getSymbol, parseAssetInfo } = useProtocol()
  const { data: proxyWhitelist } = useGetProxyWhitelist()
  ...
  const parseParams = (
    ...
    const parsed =
      "whitelist" in decoded
        ...
        : "update_collateral_multiplier" in decoded
        ? parseUpdateCollateralMultiplier(decoded.update_collateral_multiplier)
        : "spend" in decoded
  ...
  const parsePassCommand = ({ msg }: PassCommand) => {
    ...
    return "update_asset" in decodedPassCommand
      ...
      ? parseUpdatePriorityList(decodedPassCommand.update_source_priority_list)
